...
public class WorkspaceView {
	...
	private static final Logger logger = LoggerFactory.getLogger(WorkspaceView.class);

	@PostConstruct
	public void createControls(Composite parent, Workspace workspace, EMenuService menuService,
			final IEclipseContext ctx, final IEventBroker eventBroker, EModelService modelService,
			...
			public void doubleClick(DoubleClickEvent event) {
				...
				if (selection instanceof DatingProject) {
					...
					if (!isOpen) { // Make sure to reload project
						project = WorkspaceUtil.refreshDatingProject(workspace, project);
						viewer.refresh();
					}
					...
					eventBroker.send(CoreModelConstants.OPEN_PROJECT_EVENT, project);
		...
		viewer.setContentProvider(new ViewContentProvider(partStackList.get(0)));// openProjects));
		viewer.setLabelProvider(new FolderDisplayLabelProvider());
		viewer.setInput(workspace);

		menuService.registerContextMenu(viewer.getTree(), "icd2.popupmenu.leftTree");
		...
		viewer.getTree().addMouseListener(new MouseAdapter() {
			...
			public void mouseDown(MouseEvent e) {
				...
				if (e.button == 3) { // Right click?
					...
					TreeItem itemAtClick = viewer.getTree().getItem(new Point(e.x, e.y));
		...
		viewer.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				ModelObject<?, ?> item = (ModelObject<?, ?>) selection.getFirstElement();
	...
	class ViewContentProvider implements ITreeContentProvider {
		...
		public boolean hasChildren(Object element) {
			...
			if (element instanceof DatingProject) {
				...
				logger.info("Checking if dating project tree node named {} has children", dp.getName());
	...
	@Inject
	@Optional
	public void onAddNewCore(@UIEventTopic(CoreModelConstants.ON_ADD_NEW_CORE) Core newCore, Workspace workspace) {
	...
	@Inject
	@Optional
	public void onNewDatingProject(@UIEventTopic(CoreModelConstants.ON_NEW_DATING_PROJECT) DatingProject project,
			Workspace workspace) {
	...
	@Inject
	@Optional
	public void onRemovedDatingProject(@UIEventTopic(CoreModelConstants.REMOVED_DATING_PROJECT) DatingProject project,
			Workspace workspace) {
		viewer.refresh(workspace);
	...
	@Inject
	@Optional
	public void onDatingProjectOpened(@UIEventTopic(CoreModelConstants.OPEN_PROJECT_EVENT) DatingProject project,
			Workspace workspace) {
		logger.info("Dating project opened. Refreshing folder {}", project.getName());
	...
	@Inject
	@Optional
	public void onDatingProjectClosed(@UIEventTopic(CoreModelConstants.CLOSE_PROJECT_EVENT) DatingProject project,
			Workspace workspace) {
		logger.info("Dating project opened. Refreshing folder {}", project.getName());
